...
- **Achats Groupés (Group Buying) :** Logiciel de réduction basé sur le nombre de participants. Le backend associe automatiquement les acheteurs sous un même code de groupe (`group_buy_id`) ouvert pour le même produit/vendeur, validant le groupe à l'état `completed` une fois le seuil atteint.
- **Hubs Logistiques (Points de Distribution) :** Acheminement vers des points physiques prédéfinis au Cameroun (Yaoundé / Douala) pour réduire les frais de transport individuels. Suivi du statut logistique contrôlé par le vendeur et affiché en temps réel au client acheteur.
